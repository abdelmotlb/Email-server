...
export class InboxComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		InboxComponent.selectedItems = new Array(9).fill(false);
		...
		for (let index = 0; (this.page - 1) * this.messagesPerPage + index < HomePageComponent.toShow.length; index++) {
			InboxComponent.selectedItems[index] = false;
	...
	isChecked(evt: any, index: any) {
		...
		InboxComponent.selectedItems[index] = evt.target.checked;
		console.log(index + " " + InboxComponent.selectedItems);
	}
	...
	collectDeleted_request() {
		...
		// generate the string >>
		for (let index = 0; (this.page - 1) * this.messagesPerPage + index < HomePageComponent.toShow.length; index++) {
			if (InboxComponent.selectedItems[index]) {
		...
		console.log(InboxComponent.selectedItems);
	...
	collectMoved_request(index: any) {
		...
		for (let i = 0; (this.page - 1) * this.messagesPerPage + i < HomePageComponent.toShow.length; i++) {
			if (InboxComponent.selectedItems[i]) {
	...
	SortByPriority(){
		...
		this.home.priorityRequest(LoginComponent.activeUser).subscribe((res:IMessage[])=>{
			console.log(res);
			HomePageComponent.toShow = res;
		});
	}
